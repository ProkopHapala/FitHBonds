...
# Compute base paths relative to this module location
_MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
_BASE_DIR   = os.path.normpath(os.path.join(_MODULE_DIR, '../..'))
_DATA_DIR   = os.path.join(_BASE_DIR, 'data')
_TEST_DIR   = os.path.join(_BASE_DIR, 'tests/tFitREQ')

def _dat(f): return os.path.join(_DATA_DIR, f)
def _tst(f): return os.path.join(_TEST_DIR, f)
...
def run_derivative_test(
    ...
    atom_types_file=_dat("AtomTypes.dat"),
    xyz_file=_tst("HHalogens/porcessed/HF-A1_HF-D1.xyz"),
    dof_file=_tst("dofSelection_MorseSR.dat"),
    eps=1e-4,
    verbose=0,
):
...
def run_energy_imshow(model_name='ENERGY_MorseQ_PAIR',
                      xyz_file=_tst("HHalogens/HBr-D1_HBr-A1.xyz"),
                      atom_types_file=_dat("AtomTypes.dat"),
...
def run_templated_example(model_name='MODEL_MorseQ_PAIR',
                          atom_types_file=_dat("AtomTypes.dat"),
                          xyz_file=_tst("input_example.xyz"),
                          dof_file=_tst("dofSelection_MorseSR.dat")):
...
def run_baseline_example():
    ...
    driver.load_atom_types(_dat("AtomTypes.dat"))
    driver.load_data(_tst("input_example.xyz"))
    driver.load_dofs(_tst("dofSelection_MorseSR.dat"))
