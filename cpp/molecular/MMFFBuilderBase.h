...
class BuilderBase{  public:
    ...
    // === Functions copied from MMFFBuilder.h to eliminate that dependency ===
    ...
    Atoms* buildBondsAndEpairs( Atoms* mol, bool bPBC_, double Rfac=-1.35, bool bByRvdw=false, bool bDummyEpair=true, bool bAddPi=true, bool bAddECaps=true ){
        bondByRvdW = bByRvdw;
        clear();
        ...
        cleanPis();
        if( bPBC_ ){
            autoBondsPBC( Rfac,  0      , mol->n0     );
            autoBondsPBC( Rfac,  mol->n0, mol->natoms );
        }else{
            autoBonds( Rfac,  0      , mol->n0     );
